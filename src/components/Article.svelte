<script lang="ts">
	import { IconShoppingCartPlus, IconShoppingCartOff, IconCornerLeftUp } from '@tabler/icons-svelte'
	...
	// Add to cart
	$: indexOfArticleInCart = $storeShoppingCart.findIndex((articleInStore: Article) => articleInStore.id === article.id)
	...
	function addArticleToShoppingCart(): void {
		...
		if (indexOfArticleInCart !== -1) {
			...
			if (article.quantity > $storeShoppingCart[indexOfArticleInCart].quantity) {
				const copy = $storeShoppingCart.map((item: Article) => (item.id === article.id ? { ...item, quantity: item.quantity + 1 } : { ...item }))
			...
			<div class="my-3 flex max-w-[40rem] flex-wrap gap-3">
				{#each article.images as image, indexImage}
					<button on:click={() => (currentlySelectedImage = indexImage)}>
						<img src={image.image.url} alt={image.image.title} class="h-[3rem] w-[4rem] object-contain lg:h-[5rem] lg:w-[7rem]" />
					</button>
				{/each}
			</div>
		{/if}
	</div>
	<div class="flex h-full flex-col items-center justify-between xl:col-span-1 xl:py-16">
		<!-- Text Section -->
		<div class={`max-w-xl text-xl ${readMore ? '' : 'line-clamp-[8] xl:line-clamp-none'}`}>
			{article.text}
		</div>
		<button on:click={() => (readMore === true ? (readMore = false) : (readMore = true))} class="btn-sm btn mt-2 !bg-neutral xl:hidden">
			{#if readMore}
				<IconCornerLeftUp />
			{:else}
				Read More ..
		...
		<div class="mt-10 flex items-center justify-center gap-5 font-sans text-base">
			<div>
				<div>
					{new Intl.NumberFormat('en-DE', { style: 'currency', currency: 'EUR' }).format(article.price)}
				</div>
				<div>In store: {article.quantity} pcs</div>
			</div>
			{#if article.quantity > 0}
				<button on:click={addArticleToShoppingCart} class="bg-primary-600 btn rounded font-bold shadow">
